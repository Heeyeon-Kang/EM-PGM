...
# pgm_result_lasso_nonfixedK <- PGM_mvFMR_LASSO_nonfixedK(X, Y, eta_val=1, lambda=lambda_s)
...
PGM_mvFMR_LASSO_nonfixedK <- function(X, Y, n = nrow(X), P = (ncol(X)-1), m = ncol(Y), init = TRUE,
                                      eta_val, lambda, maxiter = 100){
  ...
  return(list(total = total_LASSO, total_w = optimal_w, total_density = optimal_density, 
              ...
              K = optimal_K, BIC = BIC_LASSO_s, optimal_w = optimal_w_K, optimal = optimal_OUTPUTS_LASSO_K, 
              optimal_density = optimal_density_K, optimal_BIC = optimal_BIC_LASSO_s, 
