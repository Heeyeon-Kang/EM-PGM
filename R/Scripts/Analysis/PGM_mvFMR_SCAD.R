...
####           using the EM-PGM algorithm presented in Section 5.3.            ####
###################################################################################

# The following R-code demonstrates how to implement mvFMR-SCAD using the EM-PGM algorithm 
...
# pgm_result_SCAD_nonfixedK <- PGM_mvFMR_SCAD_nonfixedK(X, Y, et_val=1, a=3.7, lambda=lambda_s)
...
PGM_mvFMR_SCAD_nonfixedK <- function(X, Y, n = nrow(X), P = (ncol(X)-1), m = ncol(Y), init = TRUE,
                                     eta_val, a, lambda, maxiter = 100){
  ...
  for(K in 1:6){
    ...
    for(z in 1:length(lambda)){
      try({
        ...
        new_Bk <- m_step_Bk_PGM_mvFMR_SCAD(X, Y, pi_, Bk, inv_sigma, w, lamb, eta_val, a, 1)
        Bk[[2]] <- new_Bk$B
        ...
        EM_computation_time[[K]][[z]] <- data.frame(iter_num = 1,
        ...
        while(theta_diff[t] >= 1e-6){
          ...
          new_Bk <- m_step_Bk_PGM_mvFMR_SCAD(X, Y, pi_, Bk, inv_sigma, w, lamb, eta_val, a, t)
          Bk[[t+1]] <- new_Bk$B
      ...
      BIC_SCAD[z] <- modified_BIC(pi_, Bk, density, w, t)
    ...
    min_SCAD <- which.min(BIC_SCAD_s[[K]])
    total_SCAD[[K]] <- total_OUTPUTS_SCAD
    optimal_OUTPUTS_SCAD[[K]] <- total_OUTPUTS_SCAD[[min_SCAD]]
    optimal_w[[K]] <- w_s[[min_SCAD]]
    optimal_density[[K]] <- density_s[[min_SCAD]]
    optimal_Bk_time_s[[K]] <- Bk_computation_time[[K]][[min_SCAD]]
    optimal_EM_time_s[[K]] <- EM_computation_time[[K]][[min_SCAD]]
  }
  
  optimal_BIC_SCAD_s <- sapply(BIC_SCAD_s, function(x) min(x, na.rm = TRUE))
  optimal_K <- which.min(optimal_BIC_SCAD_s)
  ...
  return(list(total = total_SCAD, total_w = optimal_w, total_density = optimal_density,
              total_EM_time = EM_computation_time, total_Bk_time = Bk_computation_time,
              K = optimal_K, optimal_w = optimal_w_K, BIC = BIC_SCAD_s, 
              optimal = optimal_OUTPUTS_SCAD_K, optimal_density = optimal_density_K, 
              optimal_BIC = optimal_BIC_SCAD_s, optimal_EM_time = optimal_EM_time, 
              optimal_Bk_time = optimal_Bk_time))
}