...
# pgm_result_scad_fixedK <- PGM_mvFMR_SCAD_fixedK(X, Y, K=2, eta_val=1, a=3.7, lambda=lambda_s)
...
PGM_mvFMR_SCAD_fixedK <- function(X, Y, n = nrow(X), P = (ncol(X)-1), m = ncol(Y), init = TRUE,
  ...
  optimal_Bk_time <- Bk_computation_time[[min_SCAD]]
  optimal_EM_time <- EM_computation_time[[min_SCAD]]
  
  return(list(total = total_OUTPUTS_SCAD, total_w = w_s, total_density = density_s, 
              total_EM_time = EM_computation_time, total_Bk_time = Bk_computation_time,
              optimal = optimal_OUTPUTS_SCAD, optimal_w = optimal_w, optimal_density = optimal_density, 
              BIC = BIC_SCAD, optimal_EM_time = optimal_EM_time, optimal_Bk_time = optimal_Bk_time))
}