...
# pgm_result_MCP_nonfixedK <- PGM_mvFMR_MCP_nonfixedK(X, Y, et_val=1, a=3.7, lambda=lambda_s)
...
PGM_mvFMR_MCP_nonfixedK <- function(X, Y, n = nrow(X), P = (ncol(X)-1), m = ncol(Y), init = TRUE,
  ...
  return(list(total = total_MCP, total_w = optimal_w, total_density = optimal_density,
              ...
              K = optimal_K, optimal_w = optimal_w_K, optimal = optimal_OUTPUTS_MCP_K, 
              optimal_density = optimal_density_K, optimal_BIC = optimal_BIC_MCP_s, 
              BIC = BIC_MCP_s, optimal_EM_time = optimal_EM_time, optimal_Bk_time = optimal_Bk_time))
}