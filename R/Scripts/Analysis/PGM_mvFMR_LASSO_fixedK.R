...
# pgm_result_lasso_fixedK <- PGM_mvFMR_LASSO_fixedK(X, Y, K=2, eta_val=1, lambda=lambda_s)
...
PGM_mvFMR_LASSO_fixedK <- function(X, Y, n = nrow(X), P = (ncol(X)-1), m = ncol(Y), init = TRUE,
                                   K, eta_val, lambda, maxiter = 100){
  ...
  for(z in 1:length(lambda)){
    try({
      ...
      new_Bk <- m_step_Bk_PGM_mvFMR_LASSO(X, Y, pi_, Bk, inv_sigma, w, lamb, eta_val, 1)
      Bk[[2]] <- new_Bk$B
      ...
      while(theta_diff[t] >= 1e-6){
        ...
        new_Bk <- m_step_Bk_PGM_mvFMR_LASSO(X, Y, pi_, Bk, inv_sigma, w, lamb, eta_val, t)
        Bk[[t+1]] <- new_Bk$B
    ...
    BIC_LASSO[z] <- modified_BIC(pi_, Bk, density, w, t)
  ...
  optimal_Bk_time <- Bk_computation_time[[min_LASSO]]
  optimal_EM_time <- EM_computation_time[[min_LASSO]]
  
  return(list(total = total_OUTPUTS_LASSO, total_w = w_s, total_density = density_s, 
              total_EM_time = EM_computation_time, total_Bk_time = Bk_computation_time,
              optimal = optimal_OUTPUTS_LASSO, optimal_w = optimal_w, optimal_density = optimal_density, 
              BIC = BIC_LASSO, optimal_EM_time = optimal_EM_time, optimal_Bk_time = optimal_Bk_time))
}