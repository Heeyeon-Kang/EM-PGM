...
##############################################################################
####      R-code of fitting the data using mvFMR-LASSO with known K       ####
####                 presented in Section 5.1 and 5.2.                    ####
##############################################################################

# The following R-code fits the simulation data using mvFMR-LASSO with known K
# presented in Section 5.1 and 5.2.
...
# Y <- data_generate_5.1.1(seed_number_5.3.1[1], 500)$Y
# result_lasso <- mvFMR_LASSO_fixedK(X, Y, K=2, eta=1)

source("./Data/simulation_seed_number.R")
source("./Data/simulation_data.R")
source("./Functions/functions.R")

mvFMR_LASSO_fixedK <- function(X, Y, n=nrow(X), P=(ncol(X)-1), m=ncol(Y), K, eta){
  ...
  for(z in 1:length(lambda_s)){
    try({
      ...
      w[[2]] <- e_step_w(pi_, density, n, K, 1)
      
      pi_[[2]] <- m_step_pi(w, n, 1)
      Bk[[2]] <- m_step_Bk_mvFMR_LASSO(pi_, Bk, sigma, w, lambda, eta, n, K, 1)
      sigma[[2]] <- m_step_sigma(X, Y, Bk, sigma, w, K, 1)
      ...
      theta_diff[2] <- total_diff_norm(pi_, Bk, sigma, K, 1)
      
      ## The iteration of E and M steps ##
      t <- 2
      while(theta_diff[t] >= 1e-6){
        w[[t+1]] <- e_step_w(pi_, density, n, K, t)
        
        pi_[[t+1]] <- m_step_pi(w, n, t)
        Bk[[t+1]] <- m_step_Bk_mvFMR_LASSO(pi_, Bk, sigma, w, lambda, eta, n, K, t)
        sigma[[t+1]] <- m_step_sigma(X, Y, Bk, sigma, w, K, t)
        ...
        theta_diff[t+1] <- total_diff_norm(pi_, Bk, sigma, K, t)
    ...
    BIC_LASSO[z] <- modified_BIC(pi_, Bk, density, w, n, K, t)
  ...
  min_LASSO <- which.min(replace(BIC_LASSO, which(is.infinite(BIC_LASSO)==T), NA))
  optimal_OUTPUTS_LASSO <- total_OUTPUTS_LASSO[[min_LASSO]]
  optimal_w <- w_s[[min_LASSO]]
  optimal_density <- density_s[[min_LASSO]]
  
  return(list(total=total_OUTPUTS_LASSO, optimal=optimal_OUTPUTS_LASSO, total_w=w_s, 
              total_density=density_s, optimal_w=optimal_w, optimal_density=optimal_density, BIC=BIC_LASSO))
}
