...
# result_lasso_fixedK <- mvFMR_LASSO_fixedK(X, Y, K=2, eta=1)
...
mvFMR_LASSO_fixedK <- function(X, Y, n=nrow(X), P=(ncol(X)-1), m=ncol(Y), K, eta){
  ...
  for(z in 1:length(lambda_s)){
    try({
      ...
      pi_[[2]] <- m_step_pi(w, 1)
      Bk[[2]] <- m_step_Bk_mvFMR_LASSO(pi_, Bk, inv_sigma, w, lambda, eta, 1)
      ...
      while(theta_diff[t] >= 1e-6){
        ...
        Bk[[t+1]] <- m_step_Bk_mvFMR_LASSO(pi_, Bk, inv_sigma, w, lambda, eta, t)
