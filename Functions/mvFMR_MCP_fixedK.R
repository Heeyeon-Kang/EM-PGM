...
##############################################################################
####       R-code of fitting the data using mvFMR-MCP with known K        ####
####                 presented in Section 5.1 and 5.2.                    ####
##############################################################################

# The following R-code fits the simulation data using mvFMR-MCP with known K
# presented in Section 5.1 and 5.2.
...
# result_mcp <- mvFMR_MCP_fixedK(X, Y, K=2, eta=1, a=3.7)

source("./Data/simulation_seed_number.R")
source("./Data/simulation_data.R")
source("./Functions/functions.R")

mvFMR_MCP_fixedK <- function(X, Y, n=nrow(X), P=(ncol(X)-1), m=ncol(Y), K, eta, a){
  ...
  for(z in 1:length(lambda_s)){
    try({
      ...
      Bk[[2]] <- m_step_Bk_mvFMR_MCP(pi_, Bk, sigma, w, lambda, eta, a, n, K, 1)
      ...
      while(theta_diff[t] >= 1e-6){
        ...
        Bk[[t+1]] <- m_step_Bk_mvFMR_MCP(pi_, Bk, sigma, w, lambda, eta, a, n, K, t)
    ...
    BIC_MCP[z] <- modified_BIC(pi_, Bk, density, w, n, K, t)
  ...
  return(list(total=total_OUTPUTS_MCP, optimal=optimal_OUTPUTS_MCP, total_w=w_s, 
              total_density=density_s, optimal_w=optimal_w, optimal_density=optimal_density, BIC=BIC_MCP))
}
