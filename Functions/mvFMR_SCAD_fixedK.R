...
##############################################################################
####      R-code of fitting the data using mvFMR-SCAD with known K        ####
####                 presented in Section 5.1 and 5.2.                    ####
##############################################################################

# The following R-code fits the simulation data using mvFMR-SCAD with known K
# presented in Section 5.1 and 5.2.
...
# result_scad <- mvFMR_SCAD_fixedK(X, Y, K=2, eta=1, a=3.7)
...
mvFMR_SCAD_fixedK <- function(X, Y, n=nrow(X), P=(ncol(X)-1), m=ncol(Y), K, eta, a){
  ...
  for(z in 1:length(lambda_s)){
    try({
      ...
      Bk[[2]] <- m_step_Bk_mvFMR_SCAD(pi_, Bk, sigma, w, lambda, eta, a, n, K, 1)
      ...
      while(theta_diff[t] >= 1e-6){
        ...
        Bk[[t+1]] <- m_step_Bk_mvFMR_SCAD(pi_, Bk, sigma, w, lambda, eta, a, n, K, t)
    ...
    BIC_SCAD[z] <- modified_BIC(pi_, Bk, density, w, n, K, t)
