...
# result_scad <- mvFMR_SCAD(X, Y, eta=1, a=3.7)
...
mvFMR_SCAD <- function(X, Y, n=nrow(X), P=(ncol(X)-1), m=ncol(Y), eta, a){
  ...
  for(K in 1:6){
    for(z in 1:length(lambda_s)){
      try({
        ...
        pi_[[2]] <- m_step_pi(w, 1)
        Bk[[2]] <- m_step_Bk_mvFMR_SCAD(pi_, Bk, inv_sigma, w, lambda, eta, a, 1)
        ...
        while(theta_diff[t] >= 1e-6){
          ...
          Bk[[t+1]] <- m_step_Bk_mvFMR_SCAD(pi_, Bk, inv_sigma, w, lambda, eta, a, t)
