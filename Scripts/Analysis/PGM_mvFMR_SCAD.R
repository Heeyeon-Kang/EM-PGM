...
# The following R-code demonstrates how to implement mvFMR-SCAD
# using the EM-PGM algorithm to the simulated data from Section 5.3.
...
# pgm_result_scad_nonfixedK <- PGM_mvFMR_SCAD_nonfixedK(X, Y, eta=1, a=3.7, lambda=lambda_s)
...
PGM_mvFMR_SCAD_nonfixedK <- function(X, Y, n=nrow(X), P=(ncol(X)-1), m=ncol(Y), eta, a, lambda, maxiter=100){
  ...
  for(K in 1:6){
    for(z in 1:length(lambda)){
      try({
        ...
        pi_[[2]] <- m_step_pi(w, 1)
        Bk[[2]] <- m_step_Bk_PGM_mvFMR_SCAD(pi_, Bk, inv_sigma, w, lamb, eta, a, 1)
        ...
        while(theta_diff[t] >= 1e-6){
          ...
          Bk[[t+1]] <- m_step_Bk_PGM_mvFMR_SCAD(pi_, Bk, inv_sigma, w, lamb, eta, a, t)
  ...
  return(list(total=total_SCAD, optimal=optimal_OUTPUTS_SCAD_K, total_w=optimal_w, 
              ...
              optimal_density=optimal_density_K, optimal_BIC=optimal_BIC_SCAD_s, BIC=BIC_SCAD_s))
}
