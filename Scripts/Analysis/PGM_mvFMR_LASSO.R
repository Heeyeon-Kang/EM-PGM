...
# pgm_result_lasso_nonfixedK <- PGM_mvFMR_LASSO_nonfixedK(X, Y, eta=1, lambda=lambda_s)
...
PGM_mvFMR_LASSO_nonfixedK <- function(X, Y, n=nrow(X), P=(ncol(X)-1), m=ncol(Y), eta, lambda, maxiter=100){
  ...
  return(list(total=total_LASSO, optimal=optimal_OUTPUTS_LASSO_K, total_w=optimal_w, 
              ...
              optimal_density=optimal_density_K, optimal_BIC=optimal_BIC_LASSO_s, BIC=BIC_LASSO_s))
}
