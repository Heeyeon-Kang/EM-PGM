...
flexmix_fixedK <- function(X, Y, n = nrow(X), P = (ncol(X)-1), m = ncol(Y), true, cluster, K){
  ...
  flexmix_computation_time <- list()
  col_names <- c("iter_num", "time", "acc_time")
  
  flexmix_computation_time <- as.data.frame(matrix(NA, nrow = 1, ncol = length(col_names)))
  colnames(flexmix_computation_time) <- col_names
  
  true_Bk <- true$Bk
  total_true_Bk <- matrix(as.vector(sapply(true$Bk, as.vector)), nrow = (P+1))
  
  flexmix_start_time <- as.numeric(proc.time()["elapsed"])
  ...
  flexmix_end_time <- as.numeric(proc.time()["elapsed"])
  flexmix_computation_time[1,] <- data.frame(iter_num = 1,
                                             time = flexmix_end_time-flexmix_start_time, 
                                             acc_time = flexmix_end_time-flexmix_start_time)
