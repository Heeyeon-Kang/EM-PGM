...
# pgm_result_mcp_nonfixedK <- PGM_mvFMR_MCP_nonfixedK(X, Y, eta=1, a=3.7, lambda=lambda_s)
...
PGM_mvFMR_MCP_nonfixedK <- function(X, Y, n=nrow(X), P=(ncol(X)-1), m=ncol(Y), eta, a, lambda, maxiter=100){
  ...
  for(K in 1:6){
    for(z in 1:length(lambda)){
      try({
        ...
        pi_[[2]] <- m_step_pi(w, 1)
        Bk[[2]] <- m_step_Bk_PGM_mvFMR_MCP(pi_, Bk, inv_sigma, w, lamb, eta, a, 1)
        ...
        while(theta_diff[t] >= 1e-6){
          ...
          Bk[[t+1]] <- m_step_Bk_PGM_mvFMR_MCP(pi_, Bk, inv_sigma, w, lamb, eta, a, t)
