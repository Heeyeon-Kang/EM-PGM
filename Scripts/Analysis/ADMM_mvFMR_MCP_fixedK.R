...
ADMM_mvFMR_MCP_fixedK <- function(X, Y, n=nrow(X), P=(ncol(X)-1), m=ncol(Y), e_abs=1e-6, e_rel=1e-4, K, rho, alpha, a, lambda, maxiter=100){
  ...
  for(z in 1:length(lambda)){
    try({
      ...
      pi_[[2]] <- m_step_pi(w, 1)
      Bk[[2]] <- m_step_Bk_ADMM_mvFMR_MCP(X, Y, pi_, Bk, sigma, w, e_abs, e_rel, lamb, rho, alpha, a, 1)
      ...
      while(theta_diff[t] >= 1e-6){
        ...
        Bk[[t+1]] <- m_step_Bk_ADMM_mvFMR_MCP(X, Y, pi_, Bk, sigma, w, e_abs, e_rel, lamb, rho, alpha, a, t)
