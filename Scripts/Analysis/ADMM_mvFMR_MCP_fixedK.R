...
# admm_result_mcp_fixedK <- ADMM_mvFMR_MCP_fixedK(X, Y, K=2, rho=1, alpha=1.5, a=3.7, lambda=lambda_s)
...
ADMM_mvFMR_MCP_fixedK <- function(X, Y, n = nrow(X), P = (ncol(X)-1), m = ncol(Y), init = TRUE,
  ...
  for(z in 1:length(lambda)){
    try({
      ...
      new_Bk <- m_step_Bk_ADMM_mvFMR_MCP(X, Y, pi_, Bk, sigma, w, e_abs, e_rel, lamb, rho, alpha, a, 1)
      Bk[[2]] <- new_Bk$B
