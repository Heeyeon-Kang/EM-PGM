...
# The following R-code demonstrates how to implement mvFMR-MCP using the EM-ADMM 
...
# admm_result_mcp_nonfixedK <- ADMM_mvFMR_MCP_nonfixedK(X, Y, rho=1, alpha=1.5, a=3.7, lambda=lambda_s)
...
ADMM_mvFMR_MCP_nonfixedK <- function(X, Y, n = nrow(X), P = (ncol(X)-1), m = ncol(Y), init = TRUE,
  ...
  for(K in 1:6){
    ...
    for(z in 1:length(lambda)){
      try({
        ...
        new_Bk <- m_step_Bk_ADMM_mvFMR_MCP(X, Y, pi_, Bk, sigma, w, e_abs, e_rel, lambda, rho, alpha, a, 1)
        Bk[[2]] <- new_Bk$B
        ...
        while(theta_diff[t] >= 1e-6){
          ...
          new_Bk <- m_step_Bk_ADMM_mvFMR_MCP(X, Y, pi_, Bk, sigma, w, e_abs, e_rel, lamb, rho, alpha, a, t)
          Bk[[t+1]] <- new_Bk$B
      ...
      BIC_MCP[z] <- modified_BIC_6(pi_, Bk, density, w, t)
    ...
    optimal_Bk_time_s[[K]] <- Bk_computation_time[[K]][[min_MCP]]
    optimal_EM_time_s[[K]] <- EM_computation_time[[K]][[min_MCP]]
  }
  ...
  return(list(total = total_MCP, total_w = optimal_w, total_density = optimal_density, 
              total_EM_time = EM_computation_time, total_Bk_time = Bk_computation_time,
              K = optimal_K, BIC = BIC_MCP_s, optimal_BIC = optimal_BIC_MCP_s, 
              optimal = optimal_OUTPUTS_MCP_K, optimal_w = optimal_w_K, optimal_density = optimal_density_K, 
