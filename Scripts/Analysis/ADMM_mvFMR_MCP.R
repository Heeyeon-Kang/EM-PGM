...
ADMM_mvFMR_MCP_nonfixedK <- function(X, Y, n=nrow(X), P=(ncol(X)-1), m=ncol(Y), e_abs=1e-6, e_rel=1e-4, rho, alpha, a, lambda, maxiter=100){
  ...
  for(K in 1:6){
    for(z in 1:length(lambda)){
      try({
        ...
        pi_[[2]] <- m_step_pi(w, 1)
        Bk[[2]] <- m_step_Bk_ADMM_mvFMR_MCP(X, Y, pi_, Bk, sigma, w, e_abs, e_rel, lambda, rho, alpha, a, 1)
