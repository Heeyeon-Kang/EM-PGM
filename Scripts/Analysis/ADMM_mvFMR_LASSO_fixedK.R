...
####       using the EM-ADMM algorithm presented in Section 5.1 - 5.2.         ####
###################################################################################

# The following R-code demonstrates how to implement mvFMR-LASSO using the EM-ADMM 
# algorithm to the simulated data from Section 5.1 - 5.2.
...
# admm_result_lasso_fixedK <- ADMM_mvFMR_LASSO_fixedK(X, Y, K=2, rho=1, alpha=1.5, lambda=lambda_s)
...
ADMM_mvFMR_LASSO_fixedK <- function(X, Y, n = nrow(X), P = (ncol(X)-1), m = ncol(Y), init = TRUE,
                                    e_abs = 1e-6, e_rel = 1e-4, K, rho, alpha, lambda, maxiter = 100){
