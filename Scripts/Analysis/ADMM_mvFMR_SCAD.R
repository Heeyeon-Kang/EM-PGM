...
# admm_result_scad_nonfixedK <- ADMM_mvFMR_SCAD_nonfixedK(X, Y, rho=1, alpha=1.5, a=3.7, lambda=lambda_s)
...
ADMM_mvFMR_SCAD_nonfixedK <- function(X, Y, n = nrow(X), P = (ncol(X)-1), m = ncol(Y), init = TRUE,
                                      e_abs = 1e-6, e_rel = 1e-4, rho, alpha, a, lambda, maxiter = 100){
  ...
  for(K in 1:6){
    ...
    for(z in 1:length(lambda)){
      try({
        ...
        new_Bk <- m_step_Bk_ADMM_mvFMR_SCAD(X, Y, pi_, Bk, sigma, w, e_abs, e_rel, lamb, rho, alpha, a, 1)
        Bk[[2]] <- new_Bk$B
        ...
        while(theta_diff[t] >= 1e-6){
          ...
          new_Bk <- m_step_Bk_ADMM_mvFMR_SCAD(X, Y, pi_, Bk, sigma, w, e_abs, e_rel, lamb, rho, alpha, a, t)
          Bk[[t+1]] <- new_Bk$B
  ...
  return(list(total = total_SCAD, total_w = optimal_w, total_density = optimal_density, 
              ...
              K = optimal_K, BIC = BIC_SCAD_s, optimal_w = optimal_w_K, 
              optimal_density = optimal_density_K, optimal_BIC = optimal_BIC_SCAD_s, 
              optimal = optimal_OUTPUTS_SCAD_K, optimal_EM_time = optimal_EM_time, optimal_Bk_time = optimal_Bk_time))
}